...
import { FormTitle, InputWraper, RowInput, StyledLabel, StyledSelect, SubmitButton } from "../layouts/Theme";
import { Input } from "./Ui/Input";
import Checkbox from "./Ui/Checkbox";
import contract from "../services/apiContract";
import { CreateOsSchema } from "../schemas/CreateOsSchema";
import { customSelectStyles } from "../layouts/Theme"; // caminho para o arquivo
...
export default function CreateOs(){
  ...
  // const [neighborhoodExtension, setNeighborhoodExtension] = useState("");
  // const [cityExtension, setCityExtension] = useState("");
  // const [stateExtension, setStateExtension] = useState("");
...
useEffect(() => {
 ...
 useEffect(() => {
  ...
  //         const response = await contract.getService(selectedOptionProject)
  //       // contract.getService()
  //     setSelectedOptionService("")
  ...
  // }, [selectedOptionProject]);
    ...
    // limpa campos CNH quando desmarc
  useEffect(() => {
    if (!isSameAddres) {
      // setZipCodeExtension("");
      // setStreetExtension("");
      // setNeighborhoodExtension("");
      // setCityExtension("")
      // setStateExtension( "");
      setValue( "adressNumberExtension","" )
  ...
  //   setValue("selectedOptionProject",event.target.value)
  // };
  ...
  if(data.sameAddress){
     extension = {
      ...
      number: data.adressNumberExtension
    };
    } else {
      extension = {
        ...
        city: data.city,
        state: data.state,
        number: data.adressNumber
      }
    }
  
  console.log("extension gerado:", data.sameAddress);
...
const payload = {
  ...
  consultant: {
    create: {
      ...
      phone: data.consultantPhone
    }
  },

  project_id: selectedOptionProject,
  service_id:data.selectedOptionService[0]?.value
}
...
async function getZipCodeExtension(valor) {
  ...
  try {
    ...
    const data = await response.json();
    if (!data.erro) {
      // setStreetExtension(data.logradouro || "");
      // setNeighborhoodExtension(data.bairro || "");
      // setCityExtension(data.localidade || "");
      // setStateExtension(data.uf || "");
      ...
      <RowInput>
        ...
        //  value={streetExtension} disabled readOnly
         ...
         />
            ...
            type="text"
            label="numero"
            name="adressNumberExtension"
            register={register}
            error={errors.adressNumberExtension}
            />
</RowInput>
...
{/* <Controller
  ...
  render={({ field }) => (
    <Select
      ...
      value={field.value} // array de {value,label}
      onChange={(selected) => field.onChange(selected || [])} 
      isMulti
      isDisabled={!selectedOptionProject}
      styles={customSelectStyles}
      noOptionsMessage={() => selectedOptionProject ? "Nenhum serviço disponível" : "Selecione um projeto primeiro"}
    />
  )}
/> */}
...
/>
      ...
      {/* {selectedOptionService.length > 0 && (
        <div className="p-2 border rounded bg-gray-50">
          <h4 className="font-semibold mb-1">Serviços Selecionados:</h4>
          <ul className="list-disc list-inside">
            {selectedOptionService.map((srv) => (
              <li key={srv.value}>{srv.label}</li>
            ))}
          </ul>
        </div>
    ...
    {/* </div> */}
        ...
        {/* <div>
        <StyledLabel >Serviço:</StyledLabel>
        <Select id="select" value={selectedOptionService} 
